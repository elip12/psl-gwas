...
##  kmers into longer kmers, filters kmers for correlation with pheno,
##  and creates integer mappings for samples, phenos, and kmers to
...
from preprocess_model import num_samples, create_kmer_sample_map, \
...
def main():
    ...
    # define file paths
    unique_kmers_file = join(project, 'data', 'preprocessed', 'unitig_sample_map_input.txt') #'unique_kmers.txt')
    ...
    if (not file_exists(kmer_sample_file) or not file_exists(kmer_pheno_file) 
        ...
        seqs = None #parse_input(samples_file)
        # number of samples
    ...
    if (not file_exists(similar_sample_file) or not file_exists(dissimilar_sample_file)) and file_exists(similarities_tsv):
        similar_sample(None, None, similarities_tsv, hist_orig_file,
            ...
            similar_sample_file, dissimilar_sample_file)
    # create kmer int map
    if not file_exists(kim_file):
        int_maps.create_kmer_int_map(kmer_sample_file, kim_file)
