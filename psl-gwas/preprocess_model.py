...
def create_kmer_sample_map(data, raw, q, k, upper, lower, thresh,
    ...
    kmers = [[line.split('\t')[0], *line.split('\t')[1].split(' ')] for line in data]
    kmers = [(line[0], *((sim[s], 1) for s in line[1:] if sim.get(s) is not None)) for line in kmers]
    ...
    kmers = [line for line in kmers if 0.01 * 3140 <= len(line) - 1 <= 0.85 * 3140]
